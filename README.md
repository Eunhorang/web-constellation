...
- 실제 코드 push 시각을 기준으로 한 마지막 업데이트 날짜
- 프로젝트별 최근 변경 요약과 업데이트 날짜 누적 기록
- 고정 좌표를 사용하는 접근 가능한 웹 별자리 지도
- 프로젝트명·설명 검색
- 상태·분류·태그 필터와 세 가지 정렬 방식
- 검색/필터 상태가 포함된 공유 가능한 주소
- 프로젝트가 많아지면 12개씩 이어서 보는 “더 보기” 기능
- 대표 프로젝트와 제작 중인 프로젝트 영역
- 모바일 우선 반응형 화면
- 자바스크립트 실행 전에도 제목과 프로젝트가 보이는 빌드 시 본문 미리 생성
...
│  ├─ prerender.mjs             # 배포 HTML에 실제 React 본문을 미리 생성
...
│  ├─ types/                    # 데이터 형식 설명
│  ├─ App.tsx                   # 한 페이지 전체 구성
│  ├─ main.tsx                  # 미리 만든 HTML에 React 기능을 연결하는 시작 파일
...
이 명령은 GitHub의 공개 저장소만 읽어 `projects.generated.json`의 날짜와 업데이트 기록을 갱신합니다.
...
- 저장소의 일반 활동 시각이 아니라 실제 코드가 올라온 `pushed_at`을 마지막 코드 업데이트로 사용합니다.
- 첫 동기화에서는 과거 내용을 추측하지 않고 현재 최신 날짜를 기준 기록으로 만듭니다.
- 다음 동기화부터 코드 push 시각이 달라질 때만 새 날짜와 간단한 변경 요약을 앞에 누적합니다.
- 같은 push 시각을 다시 확인하면 기록을 중복 추가하지 않습니다.
...
공개 저장소 목록 API만으로는 정확한 커밋 제목이나 변경 파일을 알 수 없습니다. 따라서 화면에는 “새 코드 반영”, “프로젝트 설명 변경”, “공개 사이트 주소 변경”처럼 실제로 확인 가능한 범위만 표시하며, 자세한 내용은 카드의 `GitHub 보기`에서 확인합니다.

나중에 GitHub 설명에서 삭제한 문구가 과거 기록에 남지 않도록, 저장소 설명 원문은 누적 요약에 복사하지 않습니다.

누적 기록은 `projects.generated.json`의 각 프로젝트 안에 있는 `updateHistory`에 저장됩니다. 이 파일은 자동 생성되므로 직접 수정하지 마세요.
...
병합, 숨김, 주소 우선순위, 검색, 필터, 업데이트 누적·중복 방지, 캐시 fallback, 비공개 저장소 제외를 자동 테스트합니다.

```bash
npm run build
```

GitHub 동기화와 SEO 파일 생성을 실행한 뒤, 실제 본문이 미리 들어간 배포용 사이트를 `dist/` 폴더에 만듭니다.
...
4. Mac은 `Command + K`, Windows는 `Ctrl + K`를 눌렀을 때 검색창으로 이동하는지 확인합니다.
...
11. 각 카드에 ‘마지막 코드 업데이트’ 날짜와 ‘최근 변경’ 한 줄이 보이는지 확인합니다.
12. 기록이 2건 이상인 기본 카드에서 ‘이전 업데이트 보기’를 키보드로 열 수 있는지 확인합니다.
13. 프로젝트가 13개 이상일 때 “프로젝트 더 보기”를 눌러 다음 카드가 나타나는지 확인합니다.
14. 개발자 도구 Console에 빨간 오류가 없는지 확인합니다.
15. iPhone·Android에서 기존 홈 화면 바로가기를 지우고 다시 추가해 새 궤도 아이콘이 보이는지 확인합니다.
...
- [ ] 모든 카드의 마지막 코드 업데이트 날짜와 최근 변경 요약이 표시됨
- [ ] 새 코드 push 뒤 동기화했을 때 기존 날짜를 지우지 않고 기록이 추가됨
